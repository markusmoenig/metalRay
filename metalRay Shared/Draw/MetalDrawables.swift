...
class MetalDrawables
{
    ...
    func drawDisk(position: float2, radius: Float, borderSize: Float = 0, onion: Float = 0, rotation: Float = 0, fillColor: float4 = float4(1,1,1,1), borderColor: float4 = float4(0,0,0,0))
    {
        ...
        let rect = Rect(position.x - data.borderSize / 2, position.y - data.borderSize / 2, data.radius * 2 + data.borderSize * 2, data.radius * 2 + data.borderSize * 2)
    ...
    func drawBox(position: float2, size: float2, rounding: Float = 0, borderSize: Float = 0, onion: Float = 0, fillColor: float4 = float4(1,1,1,1), borderColor: float4 = float4(0,0,0,0), texture: MTLTexture? = nil)
    {
        ...
        let rect = Rect(position.x - data.borderSize / 2, position.y - data.borderSize / 2, size.x + data.borderSize * 2, size.y + data.borderSize * 2, scale: 1)
        let vertexData = createVertexData(rect)
    ...
    func drawBoxPattern(position: float2, size: float2, rounding: Float = 0, borderSize: Float = 0, onion: Float = 0, rotation: Float = 0, fillColor: float4 = float4(1,1,1,1), borderColor: float4 = float4(0,0,0,0), texture: MTLTexture? = nil)
    {
        ...
        renderEncoder.setRenderPipelineState(metalView.game.metalStates.getState(state: .DrawBackPattern))
        renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 6)
    }
    
    /// Draws a box
    func drawGrid(offset: float2, size: float2, gridSize: Float, backColor: float4 = float4(0,0,0,1), gridColor: float4 = float4(1,1,1,1))
    {
        var data = GridUniform()
        data.offset = offset * float2(1, -1)
        data.screenSize = size
        data.backColor = backColor
        data.gridColor = gridColor
        data.gridSize = gridSize * scaleFactor

        let rect = Rect(0, 0, size.x, size.y, scale: 1)
        let vertexData = createVertexData(rect)
        ...
        renderEncoder.setFragmentBytes(&data, length: MemoryLayout<BoxUniform>.stride, index: 0)
        renderEncoder.setRenderPipelineState(metalView.game.metalStates.getState(state: .DrawGrid))
        renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 6)
    }
    ...
    func drawLine(startPos: float2, endPos: float2, radius: Float, borderSize: Float = 0, fillColor: float4 = float4(1,1,1,1), borderColor: float4 = float4(0,0,0,0))
    {
        ...
        let rect = Rect( minX - borderSize / 2, minY - borderSize / 2, areaWidth, areaHeight, scale: 1)
        let vertexData = createVertexData(rect)
        ...
        renderEncoder.setRenderPipelineState(metalView.game.metalStates.getState(state: .DrawLine))
        renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 6)
    }
    ...
    func drawBezier(p1: float2, p2: float2, p3: float2, width: Float = 2, borderSize: Float = 0, fillColor: float4 = float4(1,1,1,0), borderColor: float4 = float4(1,1,1,1))
    {
        ...
        renderEncoder.setRenderPipelineState(metalView.game.metalStates.getState(state: .DrawBezier))
        renderEncoder.drawPrimitives(type: .triangle, vertexStart: 0, vertexCount: 6)
    }
